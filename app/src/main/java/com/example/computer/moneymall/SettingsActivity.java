...
public class SettingsActivity extends AppCompatActivity {
    ...
    private void Retrieveinfo(){
        mRoot.child("Product").child(uploadid).addValueEventListener(new ValueEventListener() {
            @Override
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (requestCode == CropImage.CROP_IMAGE_ACTIVITY_REQUEST_CODE) {
            ...
            if (resultCode == RESULT_OK) {
                ...
                final StorageReference filepath = UserProfileImageRef.child("item" + ".jpg");
                if (resulturi != null) {
                                    ...
                                    filepath.getDownloadUrl().addOnCompleteListener(new OnCompleteListener<Uri>() {
                                        ...
                                        public void onComplete(@NonNull Task<Uri> task) {
                                            ...
                                            if (TextUtils.isEmpty(setstatus)){
                                                ...
                                                profilemap.put("discount",setstatus);
                                                profilemap.put("image",profileImageUrl);
                                                mRoot.child("Product").child(uploadid).setValue(profilemap).addOnCompleteListener(new OnCompleteListener<Void>() {
                                                    @Override
                                           ...
                                           /* mRoot.child("Product").child(uploadid).child("image").setValue(profileImageUrl).addOnCompleteListener(new OnCompleteListener<Void>() {
                                                @Override
                                                public void onComplete(@NonNull Task<Void> task) {
                                                    if (task.isSuccessful()) {
                                                        loadingbar.dismiss();
    ...
    private void updatesettings() {
        ...
        if (TextUtils.isEmpty(setstatus)){
            ...
            profilemap.put("rating",setstatus);
            mRoot.child("Product").child(uploadid).setValue(profilemap).addOnCompleteListener(new OnCompleteListener<Void>() {
                @Override
                public void onComplete(@NonNull Task<Void> task) {
                    if (task.isSuccessful()) {
    ...
    private void SendUserToMainActivity() {
        Intent loginInt = new Intent(SettingsActivity.this,Home.class);
